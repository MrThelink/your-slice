...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (headerPlaceholder) {
      fetch(componentPath + 'header.html')
        ...
        .then(data => {
          ...
          // Update auth UI state after header loads
          setTimeout(() => {
            if (window.authManager && window.updateAuthUI) {
              window.updateAuthUI(window.authManager.isAuthenticated(), window.authManager.user);
            }
          }, 100);
  ...
  function fixNavigationPaths() {
    const currentPath = window.location.pathname;
    const navLinks = document.querySelectorAll('nav a[href], .logo a[href], .auth-buttons a[href]');
  ...
  function createCheckoutDialog() {
    ...
    const currentUser = window.authManager ? window.authManager.user : null;
    ...
    if (isLoggedIn) {
      customerName = `${currentUser.firstName || currentUser.first_name || ''} ${currentUser.lastName || currentUser.last_name || ''}`.trim();
    ...
    const customerInfoSection = isLoggedIn ? `
      <div class="customer-info-section">
        <div class="customer-status-badge">
          <span class="badge-icon">✓</span>
          <span class="badge-text">Logged in as <strong>${customerName}</strong></span>
        </div>
        <h3>Delivery Address & Contact</h3>
        <form id="customerForm">
          <div class="form-group">
            <label for="customerName">Full Name *</label>
            <input type="text" id="customerName" name="name" required placeholder="Enter your full name" value="${customerName}">
          </div>
          <div class="form-group">
            <label for="customerEmail">Email Address *</label>
            <input type="email" id="customerEmail" name="email" required placeholder="Enter your email" value="${customerEmail}">
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <input type="text" id="customerAddress" name="address" required placeholder="Enter your delivery address">
          </div>
        </form>
      </div>
    ` : `
    ...
    const styles = `
      <style>
        ...
        .customer-status-badge .badge-icon {
          font-size: 1.2rem;
          color: #4caf50;
          font-weight: bold;
        }

        .customer-status-badge .badge-text {
          color: #2e7d32;
          font-size: 0.95rem;
        }
  ...
  function processOrder(dialog) {
    ...
    submitOrderToBackend(order, dialog, completeBtn);
  }

  async function submitOrderToBackend(order, dialog, completeBtn) {
    try {
      ...
      const orderData = {
        ...
        status: 'new', // Backend expects lowercase
        user_id: order.userId || null
      };

      const response = await fetch('https://mywebserver1234.norwayeast.cloudapp.azure.com/api/orders', {
        method: 'POST',
        headers: headers,
      ...
      showToast('Order placed successfully! It will appear in the admin dashboard shortly.', 'success', 5000);
    } catch (error) {

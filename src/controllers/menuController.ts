...
// 
// Table: products  
//   Columns: product_id (PRIMARY KEY), name, description, price, is_vegan
//
// Table: daily_menu (handles daily menu assignments)
//   Columns: menu_id (PRIMARY KEY), product_id (FK), menu_date, special_price
//   Relationships: daily_menu.product_id → products.product_id
//
// Common Query Pattern: 
//   JOIN daily_menu dm ON products.product_id = dm.product_id
//   WHERE dm.menu_date = [target_date]
...
export const getTodayMenu = async (req: Request, res: Response) => {
  try {
    // Query products joined with daily_menu for CURRENT_DATE
...
export const getMenuByDate = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Query products joined with daily_menu for given date
    const result = await pool.query(`
      SELECT 
        dm.menu_id,
        dm.menu_date,
...
export const getMenuByDateRange = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Query products joined with daily_menu for date range
    const result = await pool.query(`
      SELECT 
        dm.menu_id,
        dm.menu_date,
...
export const updateMenuItemPrice = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await pool.query(
      'UPDATE daily_menu SET special_price = $1 WHERE menu_id = $2 RETURNING *',
...
export const resetMenuItemPrice = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await pool.query(
      'UPDATE daily_menu SET special_price = NULL WHERE menu_id = $1 RETURNING *',
      [menuId]
    );
      ...
      for (const product_id of product_ids) {
        try {
          ...
          const existingItem = await pool.query(
            'SELECT menu_id FROM daily_menu WHERE menu_date = $1 AND product_id = $2',
            [date, product_id]
          );
          
          if (existingItem.rows.length === 0) {
            const result = await pool.query(
              'INSERT INTO daily_menu (product_id, menu_date, special_price) VALUES ($1, $2, $3) RETURNING *',
              [product_id, date, special_price || null]
            );
...
export const addToMenu = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Add item to menu
    const result = await pool.query(
      'INSERT INTO daily_menu (product_id, menu_date) VALUES ($1, $2) RETURNING menu_id',
      [product_id, date]
    );
...
export const removeFromMenu = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await pool.query(
      'DELETE FROM daily_menu WHERE menu_id = $1 RETURNING menu_id',
      [menuId]
    );

...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    setTimeout(async () => {
        ...
        if (!window.apiClient) {
            ...
            document.getElementById('ordersEmpty').innerHTML = '<p>Error: API Client not loaded</p>';
            return;
        }
...
async function loadOrders() {
    ...
    try {
        ...
        ordersTableBody.style.display = 'table';
...
function addOrderRow(order, container) {
    ...
    row.innerHTML = `
        ...
        <td><span class="status ${statusClass}">${capitalizeStatus(order.status)}</span></td>
        <td class="action-cell">
            <select class="action-select" data-order-id="${order._id}">
